...
const LogoUploader = ({ percent, stepper, step,  border = true, title: { ready, pending, success }, msg, hash}: IParamsStepItem) => {
  //timer ref
    ...
    <div className="w-full text-left flex text-gray-500 dark:text-gray-400">
      <div className="w-1/2 flex flex-col items-end px-7">
        ...
        <div className="text-[12px] flex gap-1 items-center">
          <a className="cursor-pointer hover:underline" target="_blank" href={`https://sepolia.etherscan.io/address/${hash}`}>{hash && reduceAddress(hash, 10)}</a>
        </div>
      </div>
      <div
